...
// whole seconds ([`crate::graph_time::normalize_timestamp`]). Fixed-width UTC text
// sorts lexicographically exactly as it sorts chronologically, so the store orders
// and pages on the string itself and no date arithmetic exists on the Rust side; the
...
pub struct MailHeader {
    ...
    //
    // (Normalized by `graph_time`, which every Graph surface in the crate shares.)
    pub received: String,
    pub is_read: bool,
