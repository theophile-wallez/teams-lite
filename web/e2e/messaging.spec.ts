...
test.describe("messaging", () => {
  ...
  test("replies to a message via the actions menu", async ({ page }) => {
    ...
    // AND IT OPENS NO THREAD. Threading is the OPTION beside this row, never what Reply does
    // (§ A CHAT HAS THREADS TOO): an ordinary reply is one line in the conversation, and it
    // stays in the running history where the reader put it.
    await expect(page.locator('[data-testid="threads-panel"]')).toHaveCount(0);
